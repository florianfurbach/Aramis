...
//        if(m.eval(idexp, true).isTrue()) temp= rel1;
    ...
    protected BoolExpr encodeApprox() throws Z3Exception {        
        ...
        enc = ctx.mkAnd(unionexp, union.encode(program, encodeTupleSet, ctx, Mode.KNASTER));
//        //inter:
//        RelIntersection inter=new RelIntersection(r1, r2, name);
//        interexp= ctx.mkBoolConst("inter"+getID());
//        enc = ctx.mkOr(ctx.mkAnd(ctx.mkNot(interexp),enc), ctx.mkAnd(interexp, inter.encode(program, encodeTupleSet, ctx, Mode.KNASTER)));
//       
//        //comp:
//        RelComposition comp=new RelComposition(r1, r2, name);
//        compexp= ctx.mkBoolConst("comp"+getID());
//        enc = ctx.mkOr(ctx.mkAnd(ctx.mkNot(compexp),enc), ctx.mkAnd(compexp, comp.encode(program, encodeTupleSet, ctx, Mode.KNASTER)));
//        
//        //RelTransRef:
...
//        enc = ctx.mkOr(ctx.mkAnd(ctx.mkNot(transrefexp),enc), ctx.mkAnd(transrefexp, transref.encode(program, encodeTupleSet, ctx, Mode.KNASTER)));
//        
//        //id:
...
//        enc = ctx.mkOr(ctx.mkAnd(ctx.mkNot(idexp),enc), ctx.mkAnd(idexp, enc2));
    ...
    public TupleSet getMaxTupleSet() {
        if(MaySets.get(program)==null){
            if(TemplateBasicRelation.getMaySets().get(program)!=null) r1.getMaxTupleSet();
            ...
            for(Event e : program.getCache().getEvents(FilterBasic.get(EType.ANY))){
                if(transitiveReachabilityMap.get(e)==null) 
                    maxTupleSet.add(new Tuple(e, e));
                else if(!transitiveReachabilityMap.get(e).contains(e))
                    maxTupleSet.add(new Tuple(e, e));
            }
            MaySets.put(program, maxTupleSet);
        } else maxTupleSet=MaySets.get(program);
        return maxTupleSet;    
    }
